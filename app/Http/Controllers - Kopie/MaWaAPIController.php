...
class MaWaAPIController extends Controller
{
    ...
    public function create($badge_number)
    {
        ...
        if($req_body['message'] == null || $req_body['message'] == "Error: badge already existing with one of the properties no='" . $badge_number . "' and/or coding='1319000" . $badge_number . "0000'")
        {
    ...
    public function destroy($badge_number, $transactionId)
    {
        ...
        Log::debug("9");
        if($req_body['message'] == null && $req_body2['message'] == null || $req_body2['message'] == "Error: person does not exist with this transactionId")
        {
            Log::debug("10");
            DB::table('visitorallocation')
                ->where('cardId', '=', $badge_number)
