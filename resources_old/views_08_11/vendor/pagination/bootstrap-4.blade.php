            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="page-item active" aria-current="page"><span @if(env("APP_table_Color")) class="page-link" style="background: {{ env("APP_table_Color") }}; border: {{ env("APP_table_Color") }}" @else class="page-link bg-dark border-dark" @endif>{{ $page }}</span></li>
                    @else
